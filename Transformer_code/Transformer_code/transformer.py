...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    Args:
      ...
      v: value shape == (..., seq_len_v, depth_v)       seq_len_v==seq_len_k
...
class MultiHeadAttention(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads):
        ...
        # d_model == num_heads * depth(per attension mech)
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None, mask=None):
        # --> multi attenstion --> dropout --> residual conn --> norm
        attn_output, _ = self.mha(x, x, x, mask)  # (batch_size, input_seq_len, d_model)
        attn_output = self.dropout1(attn_output, training=training)
        out1 = self.layernorm1(x + attn_output)  # (batch_size, input_seq_len, d_model)

        # --> MLP --> dropout --> residual conn --> norm
...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None, mask=None):
        ...
        # positional_coding --> dropout --> N*EncoderLayer
